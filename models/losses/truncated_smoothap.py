...
class TruncatedSmoothAP:
    ...
    def __call__(self, embeddings, positives_mask, negatives_mask):
        ...
        # Identify hard negatives for each query: the most similar negatives
        s_negatives = s_qz.detach().clone()
        s_negatives.masked_fill_(torch.logical_not(negatives_mask), np.NINF)
        hard_negatives_ndx = torch.topk(s_negatives, k=5, dim=1, largest=True, sorted=True)[1]

        query_indices = torch.arange(embeddings.size(0), device=device).view(-1, 1)
        hard_triplets = torch.cat((query_indices, closest_positives_ndx[:, :1], hard_negatives_ndx), dim=1)
